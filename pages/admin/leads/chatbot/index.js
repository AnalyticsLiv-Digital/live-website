...
const index = ({ leadDat }) => {
    const leadsData = leadDat.chatbot;
    console.log("object chatbot----------------", leadsData)
                            ...
                            <table className="min-w-full bg-gray-800 shadow-md rounded-lg overflow-hidden">
                                <thead className="bg-gray-700 text-gray-200">
                                    <tr className="text-center text-xl font-semibold h-[70px]">
                                        ...
                                        <th className="py-3 px-4">subOption</th>
                                            ...
                                            >
                                                <td className="py-4 px-6 relative"
                                                    ...
                                                    {hoveredField === `email-${key}` && (
                                                        <button
                                                            className="absolute top-1/2 right-[15%] -translate-y-1/2 bg-gray-600 text-white text-xs px-2 py-1 rounded-md"
                                                ...
                                                <td className="py-4 px-6 relative"
                                                    ...
                                                    {hoveredField === `contact-${key}` && (
                                                        <button
                                                            ...
                                                            onClick={() => handleCopy(contact.phone, `contact-${key}`)}
                                                        >
                                                            {copiedField === `contact-${key}` ? "Copied!" : "Copy"}
                                                        </button>
                                                    )}
                                                </td>
                                                <td className="py-4 px-6">{contact.option}</td>
                                                <td className="py-4 px-6">{contact.subOption}</td>
...
export async function getServerSideProps(context) {
    const res = await fetch(`${process.env.domain}/api/admin/leads?type=chatbot`);
