...
const index = ({ leadDat }) => {
    const leadsData = leadDat.contact;
                            ...
                            <table className="min-w-full bg-gray-800 shadow-md rounded-lg overflow-hidden">
                                <thead className="bg-gray-700 text-gray-200">
                                    <tr className="text-center text-xl font-semibold h-[70px]">
                                        ...
                                        <th className="py-3 px-4">Requirments</th>
                                    ...
                                    {leadsData &&
                                        leadsData?.map((contact, key) => (
                                            ...
                                            >
                                                ...
                                                <td className="py-4 px-6">{contact.purpose}</td>
                                                <td className="py-4 px-6">{contact.company}</td>
                                                {/* <td className="py-4 px-6">{contact.role ? contact.role : "-"}</td> */}
                                                <td className="py-4 px-6">{contact.requirments}</td>
...
export async function getServerSideProps(context) {
    const res = await fetch(`${process.env.domain}/api/admin/leads?type=contact`);
