...
const index = ({ leadDat }) => {
    const leadsData = leadDat.popup;
    console.log("object popup----------------", leadsData)
                                            ...
                                            >
                                                ...
                                                <td className="py-4 px-6 relative"
                                                    ...
                                                    {hoveredField === `email-${key}` && (
                                                        <button
                                                            className="absolute top-1/2 right-[5%] 2xl:right-[12%] -translate-y-1/2 bg-gray-600 text-white text-xs px-2 py-1 rounded-md"
                                                ...
                                                <td className="py-4 px-6 relative"
                                                    ...
                                                    {hoveredField === `contact-${key}` && (
                                                        <button
                                                            className="absolute top-1/2 right-0 2xl:right-[10%] -translate-y-1/2 bg-gray-600 text-white text-xs px-2 py-1 rounded-md"
...
export async function getServerSideProps(context) {
    const res = await fetch(`${process.env.domain}/api/admin/leads?type=popup`);
