...
export default async function handler(req, res) {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    try {
      ...
      const uniqueFilename = `candidate-videos/${timestamp}-${randomNum}${extension}`;

      const bucket = storage.bucket("website-bucket-uploads");
      const file = bucket.file(uniqueFilename);

      // Upload file to GCS
      await file.save(fs.readFileSync(videoFile.filepath), {
        metadata: {
      ...
      const publicUrl = `https://storage.googleapis.com/website-bucket-uploads/${uniqueFilename}`;
      ...
      try {
        if (videoFile && videoFile.filepath && fs.existsSync(videoFile.filepath)) {
          fs.unlinkSync(videoFile.filepath);
        }
      } catch (cleanupError) {
