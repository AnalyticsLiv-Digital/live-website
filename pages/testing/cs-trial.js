...
const index = ({ casestudyDat }) => {
    ...
    const handleSearch = async (e) => {
        e.preventDefault();
        if (searchQuery.trim()) {
            router.push(`/testing/cs-trial?s=${searchQuery}`);
        } else {
            router.push(`/testing/cs-trial`);
        }
    };
            ...
            <section>
                ...
                <div className='px-[6%] mb-12'>
                    <div className='flex max-md:flex-col max-md:gap-7 items-center justify-between'>
                        ...
                        <form onSubmit={handleSearch} className='flex items-center md:w-[35%] 2xl:w-[45%]'>
                            <input
                                type="search"
                                placeholder="Search"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 items-center px-[10%] lg:px-[5%] gap-14">
                    ...
                    {currentData?.length > 0 ? (
                        currentData?.map((casest, key) => <Casestudy1 key={key} casestudy={casest} />)
                    ) : (
