export default function AssessmentPanel({ model, activeCat, onSetActiveCat, onSetAnswer, onNext, onPrev }) {
    ...
    return (
        <div className="border border-gray-200/60 rounded-[32px] bg-white/85 shadow-[0_18px_60px_rgba(15,23,42,0.08)] p-5 overflow-hidden" id="assessmentPanel">
            <div className="flex justify-between gap-3.5 items-start mb-3.5">
                <div>
                    <h2 className="m-0 text-[16.5px] tracking-tight">Assessment</h2>
                ...
                <div className="flex gap-2 items-center border border-gray-200/60 bg-white/75 px-3 py-2 rounded-full text-[12.5px] text-gray-500 whitespace-nowrap" title="Current category">
                ...
                {model.map((cat, idx) => {
                    ...
                    return (
                        <div
                            ...
                            className={`border border-gray-200/60 bg-white/95 rounded-full px-3 py-2.5 text-[12.5px] font-black cursor-pointer transition-all select-none shadow-[0_10px_22px_rgba(15,23,42,0.05)] hover:translate-y-[-1px] hover:border-gray-900/20 ${idx === activeCat
            ...
            <div className="mt-4">
                {currentCat?.questions.map((qq, qIdx) => (
                    <div
                        key={qq.id}
                        className="border border-gray-200/60 bg-white/95 rounded-3xl p-4 shadow-[0_12px_28px_rgba(15,23,42,0.05)] mb-3.5 transition-all hover:translate-y-[-1px] hover:shadow-[0_18px_40px_rgba(15,23,42,0.07)]"
                    >
                        <div className="flex justify-between gap-3 items-start">
                            ...
                            <div className="text-[11.5px] font-[850] text-gray-500 border border-gray-200/60 bg-gray-900/5 px-2.5 py-1.5 rounded-full whitespace-nowrap h-fit">
                                {qq.badge}
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-4 gap-3 mt-3.5 max-md:grid-cols-2 max-sm:grid-cols-1" aria-label="Options">
                            {options.map(opt => (
                                <div
                                    ...
                                    className={`border border-gray-200/60 bg-white/95 rounded-2xl p-3 cursor-pointer flex gap-2.5 items-start min-h-[64px] shadow-[0_10px_22px_rgba(15,23,42,0.04)] transition-all select-none hover:translate-y-[-1px] hover:border-gray-900/20 ${qq.value === opt.score
            ...
            <div className="flex justify-between items-center gap-3 flex-wrap mt-4 pt-4 border-t border-gray-200/60">
                <div className="flex gap-2 items-center text-[12.5px] text-gray-500">
                    <span className="border border-gray-200/60 bg-white/95 px-2 py-1.5 rounded-[10px] text-[11.5px] text-gray-500 font-[850]">←</span>
                    <span className="border border-gray-200/60 bg-white/95 px-2 py-1.5 rounded-[10px] text-[11.5px] text-gray-500 font-[850]">→</span>
